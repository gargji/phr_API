...
router.post('/saveDemographicsWhoFormData', (req,res)=>{
    ...
    if(formState == 'new'){
        ...
        sql = `update master_patient set  prefix = '${prefix}', firstName = '${firstName}', middleName ='${middleName}', lastName = '${lastName}', suffix = '${suffix}', displayName = '${displayName}', nickName = '${nickName}',completeName = '${firstName} ${middleName} ${lastName}', previousName = '${previousName}', dateOfBirth = '${dateOfBirth}', smokingStatus = '${smokingStatus}', deceasedStatus = '${deceasedStatus}', deceasedDate = '${deceasedDate}', deceasedReason = '${deceasedReason}', maritalStatus ='${maritalStatus}', familySize = '${familySize}', monthlyIncome = '${monthlyIncome}', preferredLanguage = '${preferredLanguage}', bloodGroup = '${bloodGroup}', sex = '${sex}', sogiDeclaration = '${sogiDeclaration}', sexualOrientation = '${sexualOrientation}', age = '${age}' where guid = '${guid}';`
    }
...
router.post('/caregiverInsertion', (req, res) => {
    // console.log(req.body);
    let {formState} = req.body.formState;
    ...
    if(formState = 'new'){
        ...
        sql =  `update master_caregiver relationship='${relationship}',firstName='${firstName}',middleName='${middleName}', lastName='${lastName}', dob='${dob}', sex='${sex}', addressLine1='${addressLine1}', addressLine2='${addressLine2}', area='${area}', city='${city}' , district='${district}', state='${state}', country='${country}', landmark='${landmark}', postalCode='${postalCode}','emailId1=${emailId1}',emailId2='${emailId2}',mobilePhone'${mobilePhone}',homePhone'${homePhone}',workPhone'${workPhone}' where guid = '${guid}'`
    }
    execCommand(sql.replace(/null/g,''))
    .then(result => res.json('success'))
    .catch(err => logWriter(sql, err));
...
let sql = ``
    if(formState  == 'new'){
        sql = `insert into master_choices (guid, phrInvitationSent, hippaNoticeSent, preferredCommunication, emailNotification, voiceNotification, textNotification, pharmacy) values('${guid}', ${phrInvitationSent}, ${hippaNoticeSent}, '${preferredCommunication}', ${emailNotification}, ${voiceNotification}, ${textNotification}, '${pharmacy}')`;
...
// router.get('/submit_registration_details', (req,res)=>{
