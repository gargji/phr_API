...
router.post('/AddUpdatemaster_lab_department', (req,res)=>{
    ...
    if(id=='' || id=='0' || id==undefined){
         command = `INSERT INTO master_lab_department(HospitalId, Department, Description, LanguageId, Active) values('${HospitalId}','${Department}','${Description}','${LanguageId}',1)`;
    }
    else{
         command =`update master_lab_department set Department='${Department}',LanguageId='${LanguageId}',Description='${Description}' where id='${id}'`;
        returnmessage="U"
    }
...
router.post('/getmaster_lab_departmentData', (req,res)=>{
    ...
    const command = `SELECT * FROM master_lab_department where HospitalId='${HospitalId}'`;
