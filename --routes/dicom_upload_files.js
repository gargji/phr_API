...
router.post("/getPatientFordashBoard", async (req, res) => {
  ...
  let command = `SELECT *, (select gender from gender where conceptId = master_patient.sex)as sex, (select bloodGroup from blood_group where conceptId = master_patient.bloodGroup)as bloodGroup FROM master_patient where guid='${patientID}' and hospitalId = '${hospitalId}' and branchId = '${branchId}'`;
console.log(command);
  execCommand(command)
