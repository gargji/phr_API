...
// getSelectedResourcesForProvider
      ...
      if (result) {
        ...
        (function loop() {
          if (i < PlacesOfPraciticedata.length) {
            var command = `insert into transation_placeofpractise ( Provider_id, Hospitalname, OrganizationName, Address, phone)
...
router.post('/insertProviderContact', (req, res) => {
  ...
  execCommand(command1)
    ...
    .then(result => {
      if (result) {
        var command = `insert into provider_contact (provider_id, addressLine1, addressLine2, area, landmark, city, district, state, country, postalCode, emailId1, emailId2, emergencyContactNumber, mobilePhone, homePhone, workPhone)
         values('${guid}','${addressLine1}','${addressLine2}','${area}', '${landmark}', '${city}', '${district}' , '${state}', '${country}', '${postalcode}', '${emailId1}', '${emailId2}','${emergencyContactNumber}','${mobilePhone}','${homePhone}','${workPhone}')`
        execCommand(command)
          .then(result => res.json('success'))
          .catch(err => logWriter(command, err));
...
router.post("/createusersavedata", (req, res) => {
  ...
  var command = `INSERT INTO provider_personal_identifiers(guid,hospital_id,branchId,firstname)values('${guid}','${hospital_id}','${branchid}','${membername}'); insert into login_credentials (user_id, hospital_id, branch_id, email_id, password, user_type) values('${guid}','${hospital_id}','${branchid}', '${EmailId}','${generatePass()}' ,'${role}')`;
  execCommand(command)
    .then(result => {
      if (result) {
