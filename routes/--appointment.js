          ...
          execCommand(command)
            .then(result => {
              if (result[0].count == 0) {
                ...
                mailOptions.subject = `i Email for PHR visit`
                mailOptions.html = `<div style="font-family: Helvetica,Arial,sans-serif;min-width:1000px;overflow:auto;line-height:2">
                      <div style="margin:50px auto;width:70%;padding:20px 0">
                        ...
                        <p>Thank you for choosing Healaxy. we use follow link to click</p>
                        <a href="http://localhost:4200/registration?hopitalID=${hospital_id}&patientID=${patient_id}" target="_blank">jkadsgfhsfJKHFSGHJSGDSFHJGhj</a>
                        &aman=addNewPatient
                        Email:
                        <h2 style="width: max-content;padding: 0 10px;color: #fff;">${patient_email_id}</h2>
                        password:
                        <h2 style="background: #00466a;margin: 0 auto;width: max-content;padding: 0 10px;color: #fff;border-radius: 4px;">123</h2>
...
router.post('/SaveTask_Appointment',(req,res)=>{
    ...
    var command = `INSERT INTO transaction_task (hospital_id, branch_id, task_category, taskFor, owner, priority, duedate, notification, message)
    VALUES ('${hospitalId}','${branchId}','${task_category}','${taskFor}','${owner.guid}','${priority}','${duedate}','${Notification}','${Message}')`;
    execCommand(command)
    .then(result => res.json('success'))
    .catch(err => logWriter(command, err));
})  
...
router.post('/SendPHRInvation', (req, res) => {
  ...
  execCommand(command)
    .then(result => {
      ...
      if (result[0].count == 0) {
        mailOptions.to = `${patient_email_id}`
        mailOptions.subject = `! Email for PHR visit`
        mailOptions.html = `<div style="font-family: Helvetica,Arial,sans-serif;min-width:1000px;overflow:auto;line-height:2">
        <div style="margin:50px auto;width:70%;padding:20px 0">
          <div style="border-bottom:1px solid #eee">
            <a href="" style="font-size:1.4em;color: #00466a;text-decoration:none;font-weight:600">Healaxy</a>
          ...
          <p style="font-size:1.1em">Hi,</p>
          <p>Thank you for choosing Healaxy. we use follow link  click And Create Id</p>
          <a href="${url}registration?hopitalID=${hospital_id}&patientID=${patient_id}" target="_blank">jkadsgfhsfJKHFSGHJSGDSFHJGhj</a>
          <h2 style="width: max-content;padding: 0 10px;color: #fff;"> Email:${patient_email_id}</h2>
          <h2 style="background: #00466a;margin: 0 auto;width: max-content;padding: 0 10px;color: #fff;border-radius: 4px;">password:123</h2>
          <p style="font-size:0.9em;">Best,<br />Healaxy</p>
          <hr style="border:none;border-top:1px solid #eee" />
          <div style="float:right;padding:8px 0;color:#aaa;font-size:0.8em;line-height:1;font-weight:300">
            <p>Healaxy</p>
