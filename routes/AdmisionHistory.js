...
router.post('/Save_AddAdmission_form',(req,res)=>{
    ...
    var {id,Admissiondat,Dischargedate,AdmissionCategory,Status,Procedure,Sources,notess,Recordedby}=req.body.AddAdmission;
    let date=new Date();
    var Admissiondat = formatDate(req.body.AddAdmission.Admissiondat);
    var Dischargedate = formatDate(req.body.AddAdmission.Dischargedate);
    var DateTimes = formatDate(req.body.AddAdmission.DateTimes);
    // Admissiondat=date.toISOString().split('T')[0];
    // Dischargedate=date.toISOString().split('T')[0];
    // DateTimes=date.toISOString().split('T')[0];
    if(id=='' || id=='0' || id==undefined || id==null){

    
    const command =`INSERT INTO  master_ad_addmission (hospitalId, branchId, patientId, Hospital_Name, Admission_date, Discharge_date, Admission_Category, Admission_Reason, Status, Procedure_performed, Source, Notes,Recorded_by,DateTimes) values('${hospitalId}','${branchId}','${patientGuid}','${Hospital_Name}','${Admissiondat}','${Dischargedate}','${AdmissionCategory}','${AdmissionReason}','${Status}','${Procedure}','${Sources}','${notess}','${Recordedby}','${DateTimes}')`;
    console.log(command);
    execCommand(command)
    ...
    .catch(err => logWriter(command, err));
        ...
        const command =`Update master_ad_addmission set Hospital_Name='${Hospital_Name}', Admission_date='${Admissiondat}', Discharge_date='${Dischargedate}', Admission_Category='${AdmissionCategory}', Admission_Reason='${AdmissionReason}', Status='${Status}',Procedure_performed='${Procedure}', Source='${Sources}' ,Notes='${notess}',Recorded_by='${Recordedby}',DateTimes='${DateTimes}' where id='${id}';`;
...
WHERE 
 master_ad_addmission.hospitalId = '${hospitalId}'
 AND master_ad_addmission.branchId = '${branchId}'
 AND master_ad_addmission.PatientId = '${patientGuid}';`;
