...
router.post('/get_problem_detils',(req,res)=>{
 ...
 const command=`SELECT tp.*, ds.term AS problemName
 FROM transaction_problem tp
 JOIN description_snapshot ds ON ds.id = tp.problem
 WHERE tp.hospital_id = '${hospitalId}'
   AND tp.branch_id = '${branchId}'
   AND tp.patient_id = '${patientGuid}'
   AND tp.Resolve_status = 0
   AND tp.problemStatus = 1`
...
router.post('/save_TreatmentProblem',(req,res)=>{
    ...
    var recorded_date=req.body.problemform.recorded_date
    var Recordedby=req.body.problemform.Recordedby
   ...
   if(id=='' || id=='0' || id==undefined || id==null){
     const command =`INSERT INTO  transaction_problem(hospital_id, branch_id, patient_id, problem, laterality, onsetDate, resolutionDate, priority, source, notes,Diagnosis,problemStatus,Recordedby,recorded_date,transaction_time) values('${hospitalId}','${branchId}','${patientguid}','${problemname}','${Laterality}','${DateOfOnSet}','${last_resolution}','${Priority}','${Source}','${notes}','${Diagnosis}','${problemstataus}','${Recordedby}','${recorded_date}',now())`;
    ...
    const command =`Update transaction_problem set problem='${problemname}', laterality='${Laterality}', onsetDate='${DateOfOnSet}', resolutionDate='${last_resolution}',source='${Source}', notes='${notes}',Recordedby='${Recordedby}',recorded_date='${recorded_date}',transaction_time=now() where id='${id}';`;
