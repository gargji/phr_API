...
router.post('/AddUpdateTransaction_alcohol_and_substances_use', (req, res) => {
    ...
    var DrugUse_alcoholuse = req.body.data.DrugUse_alcoholuse
    var formType=req.body.data.formType
    var SubstanceUseType = req.body.data.SubstanceUseType
    if(formType=='1'){
         SubstanceUseType = 'fb16d6ed-91cf-4168-9043-c3bd495bda2fg'
    }else{
    ...
    var HardLiquor=req.body.data.HardLiquor
    ...
    if (id == '' || id == '0' || id == undefined || id == null) {
        ...
        command = `INSERT INTO transaction_alcohol_and_substances_use(patientId, branchId, HospitalId, DrugUse_alcoholuse,formType, SubstanceUseType, UsePerweek, SubstanceUseCounselling,HardLiquor, SubstanceUseNotes, AlcoholUse,Glassesofwine, Cansofbeer, ShotsofLiquor, StandardDrinksOrEquivalent, AlcoholCounselling, AlcoholNotes,recorded_by,recorded_date,notes_date,transaction_time) values
        ('${Patientguid}','${branchId}','${hospitalId}','${DrugUse_alcoholuse}','${formType}','${SubstanceUseType}','${UsePerweek}','${SubstanceUseCounselling}','${HardLiquor}','${SubstanceUseNotes}','${AlcoholUse}','${Glassesofwine}','${Cansofbeer}','${ShotsofLiquor}','${StandardDrinksOrEquivalent}','${AlcoholCounselling}','${AlcoholNotes}','${recorded_by}','${recorded_date}',now(), now())`;
    }
    else {
        command = `update transaction_alcohol_and_substances_use set DrugUse_alcoholuse='${DrugUse_alcoholuse}',SubstanceUseType='${SubstanceUseType}',UsePerweek='${UsePerweek}',HardLiquor='${HardLiquor}',SubstanceUseCounselling='${SubstanceUseCounselling}',SubstanceUseNotes='${SubstanceUseNotes}',AlcoholUse='${AlcoholUse}',recorded_by='${recorded_by}',notes_date=now(),recorded_date='${recorded_date}',Glassesofwine='${Glassesofwine}',Cansofbeer='${Cansofbeer}',ShotsofLiquor='${ShotsofLiquor}',StandardDrinksOrEquivalent='${StandardDrinksOrEquivalent}',AlcoholCounselling='${AlcoholCounselling}',AlcoholNotes='${AlcoholNotes}',transaction_time=now() where id='${id}'`;
        returnmessage = "U"
    }
        ...
        .then(result =>{
            if(result){
                if(id=='' || id=='0' || id==undefined || id==null){
                    ...
                    command=`INSERT INTO  allergy_notes(patientId,hospitalId,doctorId,notes,allergyId,categeory,date) values('${Patientguid}','${hospitalId}','${recorded_by}','${SubstanceUseNotes}','${Aid}','AlcoholAndSubstancesUse',now())`
            
                }else{
                    var notes_date=req.body.data.notes_date
                    console.log('notesdate',notes_date);
                    command=`update allergy_notes set  notes='${SubstanceUseNotes}',date=now() where date='${notes_date}'`
...
router.post('/InsertQuestionAnswerTransactiontable', (req, res) => {
    ...
    getQuestionAnswerPoints(answerQuestionCage, (points) => {
        ...
        const command = `INSERT INTO transaction_question_answertable_alcohol_and_substances_use(patientId, branchId, HospitalId,typeForm, points,recorded_by,TransactionTime) values('${Patientguid}','${branchId}','${hospitalId}','${typeForm}','${points}','${recorded_by}',now())`;
...
router.post('/get_transactionPoints', (req, res) => {
    console.log(req.body.text);
    var patientId = req.body.patientguid
    const command = `select * from transaction_question_answertable_alcohol_and_substances_use where patientId='${patientId}' order by id desc;`;
...
router.get('/getMasterBoolean', (req, res) => {
    // console.log(req.body.text);
    // const display_name = req.body.text
...
router.get('/getmaster_form_type_alcohol', (req, res) => {
    // console.log(req.body.text);
    // const display_name = req.body.text

    const command = `select * from master_form_type_alcohol;`;
...
router.get('/getMaster_substance_use_drugs', (req, res) => {
    const command = `select * from master_substance_use_drugs;`;
    console.log(command);
    execCommand(command)
        .then(result => res.json(result))
        .catch(err => logWriter(command, err));
...
router.post('/DeleteALcholospointData', (req, res) => {
    console.log(req.body.data);
    var id = req.body.data.id
    const command = `delete from transaction_question_answertable_alcohol_and_substances_use where id='${id}'`;
